...
원본 데이터에서 리뷰와 평점을 추출하여 평점에서 3점을 제외하고, 1점과 2점은 부정(0)으로, 4점과 5점은 긍정(1)으로 분류했다. 먼저 마침표가 2개이상(ex : 하...작가님)은 공백으로 대체하고 중복값과 결측치를 제거했다. 또한, 리뷰의 길이가 15자 미만이거나 500자를 초과하는 경우 해당 리뷰를 제거했다. 긍정과 부정으로 분류한 데이터는 78,153건이 있었고, 중복값을 제거한 결과 64,201건으로 데이터가 줄어들었다. 이로 인해 13,952건의 데이터가 삭제되었고 결측치는 없었다. 최종적으로 남은 데이터는 <b>46,310</b>개이다.  
...
50자 이하에 많은 리뷰가 몰려있다는것을 알 수 있다. Google Play Store의 리뷰가 500자까지만 허용되고 15자 미만의 짧은 리뷰는 매우 제한적이며 정보가 부족하다고 판단하여 총 17,551개를 제거했다.
<br>
...
## 4. 배운점
맞춤법과 띄어쓰기 교정을 했더라면 Accuracy가 더 높게 나오지 않았을까하는 생각이 든다. 
